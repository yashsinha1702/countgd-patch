...
def create_patches(tensor, mask=None, patch_size=224, overlap=32):
    ...
    patches = torch.stack(patches) if patches else tensor.unsqueeze(0)
    if mask is not None:
        patch_masks = torch.stack(patch_masks) if patch_masks else mask.unsqueeze(0)
...
def adjust_targets_for_patch(targets, patch_box):
    ...
    # Adjust boxes to patch coordinates
    boxes = targets['boxes'].clone()
    
    # Convert to patch coordinates
    boxes[:, [0, 2]] = boxes[:, [0, 2]] - x1
    boxes[:, [1, 3]] = boxes[:, [1, 3]] - y1
    
    # Filter boxes that are within the patch
    valid_boxes = (boxes[:, 0] < (x2-x1)) & (boxes[:, 1] < (y2-y1)) & \
                 (boxes[:, 2] > 0) & (boxes[:, 3] > 0)
    
    # Clip boxes to patch boundaries
    boxes[:, 0::2].clamp_(min=0, max=x2-x1)
    boxes[:, 1::2].clamp_(min=0, max=y2-y1)
    
    # Update targets with valid boxes only
    adjusted_targets['boxes'] = boxes[valid_boxes]
    
    # Adjust other target attributes that depend on box count
    for k, v in targets.items():
...
def train_one_epoch(
    ...
    def log_message(message, level="info"):
        """Helper function to handle logging with or without a logger"""
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header, logger=logger):
        try:
            ...
            if not torch.isfinite(samples_tensor).all():
                log_message("Non-finite values in input tensor, skipping batch", "warning")
                continue
            ...
            if any(not torch.isfinite(ex).all() for ex in exemplars):
                log_message("Non-finite values in exemplars, skipping batch", "warning")
                continue
            ...
            for idx in range(batch_size):
                # Create patches for current image
                if mask is not None:
                    patches, patch_masks, patch_locations = create_patches(
                        samples_tensor[idx], mask[idx], patch_size, overlap
                    )
                else:
                    patches, _, patch_locations = create_patches(
                        samples_tensor[idx], None, patch_size, overlap
                    )
                
                # Validate patches
                if not torch.isfinite(patches).all():
                    log_message(f"Non-finite values in patches for image {idx}, skipping", "warning")
                    continue
                
                # Get shot number for current image
                min_exemplars_in_batch = exemplars[idx].shape[0]
                shot_num = min(3, min_exemplars_in_batch)
                
                # Skip if no exemplars in training with exemplar only mode
                if args.train_with_exemplar_only and shot_num == 0:
                    continue
                    
                # Handle modality dropout
                ...
                # Process each patch
                for patch_idx, (patch, patch_loc) in enumerate(zip(patches, patch_locations)):
                    patch = patch.unsqueeze(0)  # Add batch dimension
                    
                    # Create NestedTensor for patch if needed
                    if mask is not None:
                        patch_mask = patch_masks[patch_idx].unsqueeze(0)
                        patch = utils.NestedTensor(patch, patch_mask)
                    
                    # Adjust targets for current patch
                    patch_targets = adjust_targets_for_patch(targets[idx], patch_loc)
                    patch_targets = {k: v.to(device) if isinstance(v, torch.Tensor) else v 
                                   for k, v in patch_targets.items()}
                    
                    # Validate target boxes
                    if len(patch_targets['boxes']) > 0:
                        if not torch.isfinite(patch_targets['boxes']).all():
                            log_message(f"Non-finite values in target boxes for patch {patch_idx}, skipping", "warning")
                            continue
                    
                    # Skip patches with no objects if desired
                    if len(patch_targets['boxes']) == 0 and getattr(args, 'skip_empty_patches', False):
                        continue
                    
                    with autocast(enabled=args.amp):
                        outputs = model(patch, [current_exemplars], 
                                     [labels_uncropped[idx]], 
                                     captions=[current_caption])
                        
                        # Validate model outputs
                        if not all(torch.isfinite(v).all() for v in outputs.values() if isinstance(v, torch.Tensor)):
                            log_message(f"Non-finite values in model outputs for patch {patch_idx}, skipping", "warning")
                            continue
                            
                        loss_dict = criterion(outputs, [patch_targets], 
                                           [cap_list[idx]], [current_caption])
                        
                        # Validate losses
                        if not all(torch.isfinite(v).all() for v in loss_dict.values()):
                            log_message(f"Non-finite loss values for patch {patch_idx}, skipping", "warning")
                            continue
                    ...
                    if not torch.isfinite(losses).all():
                        log_message(f"Non-finite final loss for patch {patch_idx}, skipping", "warning")
                        continue
                    ...
                    # Update batch loss dictionary
                    loss_dict_reduced = utils.reduce_dict(loss_dict)
                    for k, v in loss_dict_reduced.items():
                        batch_loss_dict[k] += v.item()
                    total_loss += losses.item()
            
            # Skip gradient update if no valid patches were processed
            if total_loss == 0:
                log_message("No valid patches in batch, skipping optimizer step", "warning")
                continue
                
            # Average losses over patches and images
            ...
            log_message(f"Error processing batch: {str(e)}", "error")
            continue
