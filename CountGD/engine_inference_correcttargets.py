...
def train_one_epoch(
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header, logger=logger):
        ...
        # Adaptive cropping
        crops, crop_boxes = adaptive_crop_batch(samples, targets)
        
        # Adjust targets for cropped patches
        adjusted_targets = adjust_targets_for_crops(targets, crop_boxes)

        with autocast(enabled=args.amp):
            outputs = model(crops, exemplars, labels_uncropped, captions=captions)
            loss_dict = criterion(outputs, adjusted_targets, cap_list, captions)
...
def adjust_targets_for_crops(targets, crop_boxes):
    adjusted_targets = []
    for target, crop_box in zip(targets, crop_boxes):
        ...
        if not valid_boxes.any():
            ...
            adjusted_target = {
                ...
                'orig_size': torch.tensor([y2-y1, x2-x1]),
                'size': torch.tensor([y2-y1, x2-x1])
            }
        else:
            adjusted_target = {
...
def adaptive_crop_batch(samples, targets, crop_size=224):
    ...
    for img, target in zip(samples_tensor, targets):
        boxes = target['boxes']
    ...
    padded_crops = [F.pad(crop, (0, max_w - crop.shape[2], 0, max_h - crop.shape[1]), value=0) for crop in crops]

    return torch.stack(padded_crops), crop_boxes
