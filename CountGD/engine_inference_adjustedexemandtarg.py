...
def train_one_epoch(
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header, logger=logger):
        ...
        # Calculate minimum exemplars and shot number
        ...
        # Handle exemplar-only training
        if hasattr(args, 'train_with_exemplar_only') and args.train_with_exemplar_only:
            if shot_num == 0:
                continue
            model.drop_text = True
            captions = [cap.replace(cap_list[t["labels_uncropped"][0]][0] + " ", "") for cap, t in zip(captions, targets)]
        else:
            model.drop_text = False

        # Modality dropout
        if hasattr(args, 'modality_dropout') and args.modality_dropout and shot_num > 0:
        ...
        # Update exemplars based on shot number
        ...
        for i in range(batch_size):
            # Process crops for a single image
            if isinstance(samples, utils.NestedTensor):
                crops, crop_boxes = adaptive_crop_single_image(samples.tensors[i], targets[i])
            else:
                crops, crop_boxes = adaptive_crop_single_image(samples[i], targets[i])
            
            # Adjust targets for cropped patches
            adjusted_targets = adjust_targets_for_crops([targets[i]], [crop_boxes])[0]
            
            # Prepare inputs for the model
            crop_batch = crops.to(device)  # Shape: [num_crops, 3, 224, 224]
            num_crops = len(crops)
            
            # Get exemplar boxes and adjust them for each crop
            exemplar_boxes = exemplars[i]  # Shape: [num_exemplars, 4]
            num_exemplars = exemplar_boxes.size(0)
            
            # Prepare exemplar boxes for each crop
            exemplar_list = []
            for crop_idx, crop_box in enumerate(crop_boxes):
                # Get crop coordinates
                x1, y1, x2, y2 = crop_box
                
                # Adjust exemplar boxes relative to the crop
                adjusted_boxes = exemplar_boxes.clone()
                adjusted_boxes[:, [0, 2]] = torch.clamp(adjusted_boxes[:, [0, 2]] - x1, min=0, max=x2-x1)
                adjusted_boxes[:, [1, 3]] = torch.clamp(adjusted_boxes[:, [1, 3]] - y1, min=0, max=y2-y1)
                
                # Only keep valid boxes (where width and height > 0)
                valid_mask = (adjusted_boxes[:, 2] > adjusted_boxes[:, 0]) & (adjusted_boxes[:, 3] > adjusted_boxes[:, 1])
                valid_boxes = adjusted_boxes[valid_mask]
                
                if len(valid_boxes) > 0:
                    exemplar_list.append(valid_boxes.to(device))
                else:
                    # If no valid boxes, create a dummy box
                    exemplar_list.append(torch.zeros((1, 4), device=device))
            
            caption_batch = [captions[i]] * num_crops
            label_batch = [labels_uncropped[i]] * num_crops
            
            try:
                with autocast(enabled=args.amp):
                    outputs = model(crop_batch, exemplar_list, label_batch, captions=caption_batch)
                    loss_dict = criterion(outputs, adjusted_targets, [cap_list[i]] * num_crops, caption_batch)
                    weight_dict = criterion.weight_dict
                    losses = sum(loss_dict[k] * weight_dict[k] for k in loss_dict.keys() if k in weight_dict)
            except Exception as e:
                print(f"Error in forward pass:")
                print(f"crop_batch shape: {crop_batch.shape}")
                print(f"num exemplar crops: {len(exemplar_list)}")
                for idx, exc in enumerate(exemplar_list):
                    print(f"exemplar_list[{idx}] shape: {exc.shape}")
                print(f"len(label_batch): {len(label_batch)}")
                print(f"len(caption_batch): {len(caption_batch)}")
                print(f"adjusted_targets type: {type(adjusted_targets)}")
                if isinstance(adjusted_targets, list):
                    print(f"adjusted_targets length: {len(adjusted_targets)}")
                raise e
...
def adaptive_crop_single_image(img, target, num_crops=4, crop_size=224):
    boxes = target['boxes']
    crops = []
    crop_boxes = []
    ...
    for _ in range(num_crops):
        if len(boxes) == 0:
            ...
            mean_size = torch.mean(boxes[:, 2:] - boxes[:, :2], dim=0)
            crop_size = int(torch.max(mean_size).item() * 1.5)
            crop_size = max(224, min(crop_size, min(img.shape[1:])))
            
            box_idx = torch.randint(0, len(boxes), (1,)).item()
            box = boxes[box_idx]
            cx, cy = (box[:2] + box[2:]) / 2
            
            x1 = max(0, int(cx - crop_size//2))
            y1 = max(0, int(cy - crop_size//2))
            x2 = min(img.shape[2], x1 + crop_size)
            y2 = min(img.shape[1], y1 + crop_size)

        crop = img[:, y1:y2, x1:x2]
        crops.append(crop)
        crop_boxes.append([x1, y1, x2, y2])

    return torch.stack(crops), crop_boxes


def adjust_targets_for_crops(targets, crop_boxes_list):
    adjusted_targets_list = []
    for target, crop_boxes in zip(targets, crop_boxes_list):
        adjusted_targets = []
        for crop_box in crop_boxes:
            crop_box_tensor = torch.tensor(crop_box, device=target['boxes'].device)
            x1, y1, x2, y2 = crop_box_tensor
            ...
            # Filter out boxes that are no longer in the crop
            valid_boxes = (adjusted_boxes[:, 2] > adjusted_boxes[:, 0]) & (adjusted_boxes[:, 3] > adjusted_boxes[:, 1])
            
            # Handle case where no valid boxes remain
            if not valid_boxes.any():
                # Create an empty tensor with the correct shape
                empty_boxes = torch.zeros((0, 4), device=boxes.device)
                empty_labels = torch.zeros((0,), dtype=target['labels'].dtype, device=target['labels'].device)
                empty_area = torch.zeros((0,), device=boxes.device)
                empty_iscrowd = torch.zeros((0,), dtype=torch.bool, device=boxes.device) if 'iscrowd' in target else None
                
                adjusted_target = {
                    'boxes': empty_boxes,
                    'labels': empty_labels,
                    'area': empty_area,
                    'iscrowd': empty_iscrowd,
                    'orig_size': torch.tensor([y2-y1, x2-x1], device=boxes.device),
                    'size': torch.tensor([y2-y1, x2-x1], device=boxes.device)
                }
            else:
                adjusted_target = {
                    'boxes': adjusted_boxes[valid_boxes],
                    'labels': target['labels'][valid_boxes],
                    'area': (adjusted_boxes[:, 2] - adjusted_boxes[:, 0]) * (adjusted_boxes[:, 3] - adjusted_boxes[:, 1])[valid_boxes],
                    'iscrowd': target['iscrowd'][valid_boxes] if 'iscrowd' in target else None,
                    'orig_size': torch.tensor([y2-y1, x2-x1], device=boxes.device),
                    'size': torch.tensor([y2-y1, x2-x1], device=boxes.device)
                }
            
            if 'image_id' in target:
                adjusted_target['image_id'] = target['image_id']
            adjusted_targets.append(adjusted_target)
        
        adjusted_targets_list.append(adjusted_targets)
    
    return adjusted_targets_list
