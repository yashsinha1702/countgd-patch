...
def train_one_epoch(
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header, logger=logger):
        try:
            ...
            all_losses = []  # Store all losses for batch backward
            ...
            for idx in range(batch_size):
                ...
                for patch_idx, (patch, patch_loc) in enumerate(zip(patches, patch_locations)):
                    ...
                    if len(patch_targets['boxes']) == 0 and getattr(args, 'skip_empty_patches', False):
                        continue
                    if "labels" not in patch_targets:
                        print(f"Missing 'labels' key in patch_targets: {patch_targets.keys()}")
                        print(f"Patch targets content: {patch_targets}")
                    ...
                    # Instead of immediate backward, accumulate the loss
            ...
            # Combine all losses and do a single backward pass
            combined_loss = sum(all_losses) / len(all_losses)
